...
class _PLUPageState extends State<PLUCalculatorScreen> {
  ...
  void _checkTour() {
    ...
    if (tourProvider.isTourActive && tourProvider.currentStep == 27 && !_tourShowing) {
      _waitForKeypadAndShowTour();
    }
  }
  ...
  void _onTourStateChanged() {
    ...
    if (tourProvider.currentStep == 27 && !_tourShowing) {
      _waitForKeypadAndShowTour();
    }
  }

  Future<void> _waitForKeypadAndShowTour() async {
    try {
      await foodItemsFuture;
    } catch (_) {}
    if (!mounted) return;
  ...
  TargetFocus? _buildKeypadTarget(TourProvider tourProvider) {
    final ctx = TourKeys.calcKeypadKey.currentContext;
    if (ctx == null) return null;
    final box = ctx.findRenderObject() as RenderBox?;
    if (box == null || !box.hasSize) return null;
    final pos = box.localToGlobal(Offset.zero);
    final size = box.size;
    final shrunkHeight = (size.height * 0.7).clamp(85.0, 300.0);
    final spotlightSize = Size(size.width, shrunkHeight);
    return TargetFocus(
      identify: "calc_keypad",
      targetPosition: TargetPosition(spotlightSize, pos),
        ...
        TargetContent(
          ...
          builder: (context, controller) {
            return tourProvider.buildTourTooltip(
              context: context,
              step: 28,
              title: AppLocale.tourTitleCalcKeypad.getString(context),
              description: AppLocale.tourDescCalcKeypad.getString(context),
  ...
  void _showTour() {
    ...
    final targets = [
      TargetFocus(
        identify: "calc_display",
        keyTarget: TourKeys.calcDisplayKey,
        alignSkip: Alignment.topRight,
        shape: ShapeLightFocus.RRect,
        radius: 12,
        contents: [
          TargetContent(
            ...
            builder: (context, controller) {
              return tourProvider.buildTourTooltip(
                context: context,
                step: 27,
                title: AppLocale.tourTitleCalcDisplay.getString(context),
                description: AppLocale.tourDescCalcDisplay.getString(context),
                onNext: () => controller.next(),
                onSkip: () {
                  tourProvider.stopTour();
                  controller.skip();
      ...
      _buildKeypadTarget(tourProvider),
    ...
    if (validTargets.isEmpty) {
      _tourShowing = false;
      if (tourProvider.isTourActive) tourProvider.setStep(29);
      return;
    }
    ...
    if (value == 'C') {
      ...
      if (enteredCode.isNotEmpty) {
        ...
        Fluttertoast.showToast(
          msg: AppLocale.noItemExistsWithCode.getString(context),
  ...
  Future<List<Map<String, dynamic>>> fetchFoodItems() async {
    try {
      ...
      List<Map<String, dynamic>> items = products
          .map((item) => {'name': PriceUtils.safeStringConversion(item.name), 'price': PriceUtils.safeStringConversion(item.price), 'foodCode': PriceUtils.safeStringConversion(item.foodCode)})
          .toList();
  ...
  void checkFoodItem(String foodCode) async {
    ...
    try {
      ...
      Fluttertoast.showToast(
        msg: "${AppLocale.noItemExistsWithCodePrefix.getString(context)}$foodCode",
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.CENTER,
        backgroundColor: primaryColor,
        textColor: Colors.white,
      ...
      Fluttertoast.showToast(
        msg: AppLocale.errorLoadingItems.getString(context),
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.CENTER,
