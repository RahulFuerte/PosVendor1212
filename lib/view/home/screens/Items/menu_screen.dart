...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Future<void> _deleteProduct(ProductModel p) async {
    final ok = await _showDeleteConfirm(
      AppLocale.deleteProduct.getString(context),
      'Delete "${p.name}"?',
    );
    if (ok != true) return;
    try {
      await _productService.deleteProduct(p.id!);
      SnackBarUtils.showSuccess(context, AppLocale.deleteProduct.getString(context));
      _loadData();
    } catch (e) {
  ...
  Future<void> _deleteCategory(CategoryModel c) async {
    final ok = await _showDeleteConfirm(
      AppLocale.deleteCategory.getString(context),
      'Delete "${c.name}"?\nProducts in this category may be affected.',
    );
    if (ok != true) return;
    try {
      await _categoryService.deleteCategory(c.id!);
      SnackBarUtils.showSuccess(context, AppLocale.deleteCategory.getString(context));
      _loadData();
    } catch (e) {
      SnackBarUtils.showError(context, 'Error: $e');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: appbar1))
          : Consumer<SubscriptionProvider>(
              builder: (context, subProvider, _) {
  ...
  Widget _buildTabSwitcher() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _tabButton(AppLocale.products.getString(context), 0),
            _tabButton(AppLocale.categoriesTab.getString(context), 1),
          ],
        ),
      ),
    );
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: selectedTab == 0
              ? AppLocale.searchProducts.getString(context)
              : AppLocale.searchCategories.getString(context),
          ...
          enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: Colors.grey.shade300)),
          focusedBorder:
              OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: appbar1)),
        ),
      ),
    );
  }
  ...
  Widget _buildList() {
    if (selectedTab == 0) {
      if (filteredProducts.isEmpty) return Center(child: MyText(text: AppLocale.noProductsFound.getString(context)));
      ...
      if (filteredCategories.isEmpty) return Center(child: MyText(text: AppLocale.noCategoriesFound.getString(context)));
      return ListView.separated(
        padding: const EdgeInsets.fromLTRB(16, 0, 16, 100),
