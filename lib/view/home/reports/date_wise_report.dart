                        ...
                        itemBuilder: (context, index) {
                          ...
                          try {
                            if (displayDate.contains('-')) {
                              ...
                              final year =
                                  int.parse(displayDate.substring(0, 4));
                              final month =
                                  int.parse(displayDate.substring(4, 6));
                              final day =
                                  int.parse(displayDate.substring(6, 8));
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Column(
                                        ...
                                        children: [
                                          _buildBadge(
                                              '${date['totalBills']} BILLS',
                                              Icons.receipt_rounded,
                                              Colors.blue),
                                          const SizedBox(height: 12),
                                          MyText(
                                            text: AppLocale.dailySales
                                                .getString(context)
                                                .toUpperCase(),
                                            color: Colors.grey[400],
                                            fontSize: 9,
  ...
  Future<void> _fetchDateData() async {
    ...
    try {
      ...
      final processedData = report.map((item) {
        ...
        final int bills =
            (item['count'] ?? item['totalBills'] ?? item['totalOrders'] ?? 0)
                .toInt();
        ...
        return {
          'date': item['_id'] ??
              item['date'], // API usually groups by _id (date string)
          'totalBills': bills,
          'amount': amount,
        };
      }).toList();
