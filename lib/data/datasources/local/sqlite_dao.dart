...
class SQLiteDAO implements DatabaseService {
  ...
  void _initializePreparedStatements() {
    _preparedStatements.addAll({
      ...
      'getFoodItemsByAdminAndDeptPaginated':
          'SELECT * FROM food_items WHERE admin_uid = ? AND department = ? ORDER BY name ASC LIMIT ? OFFSET ?',
      ...
      'searchFoodItemsLike':
          'SELECT * FROM food_items WHERE admin_uid = ? AND (name LIKE ? OR food_code LIKE ? OR description LIKE ?) ORDER BY name ASC LIMIT ?',
      'searchFoodItemsLikeWithDept':
          'SELECT * FROM food_items WHERE admin_uid = ? AND department = ? AND (name LIKE ? OR food_code LIKE ? OR description LIKE ?) ORDER BY name ASC LIMIT ?',

      // Price range queries for better performance
      'getFoodItemsByPriceRange':
          'SELECT * FROM food_items WHERE admin_uid = ? AND price BETWEEN ? AND ? ORDER BY price ASC LIMIT ?',
      'getFoodItemsByDeptAndPriceRange':
          'SELECT * FROM food_items WHERE admin_uid = ? AND department = ? AND price BETWEEN ? AND ? ORDER BY price ASC LIMIT ?',

      // Stock level queries
      'getFoodItemsByStockLevel':
          'SELECT * FROM food_items WHERE admin_uid = ? AND stocks >= ? ORDER BY stocks ASC LIMIT ?',
      ...
      'getDepartmentsPaginated':
          'SELECT * FROM departments WHERE admin_uid = ? AND status = ? ORDER BY name ASC LIMIT ? OFFSET ?',
      ...
      'getBillsByAdminAndDateRange':
          'SELECT * FROM bills WHERE admin_uid = ? AND bill_date >= ? AND bill_date <= ? ORDER BY bill_date DESC',
      ...
      'getBillsCountByDateRange':
          'SELECT COUNT(*) as count FROM bills WHERE admin_uid = ? AND bill_date >= ? AND bill_date <= ?',
      'getBillsByAmountRange':
          'SELECT * FROM bills WHERE admin_uid = ? AND total_amount BETWEEN ? AND ? ORDER BY bill_date DESC LIMIT ?',
      ...
      'getPendingDepartments':
          'SELECT * FROM departments WHERE admin_uid = ? AND sync_status = ? ORDER BY updated_at ASC',
  ...
  Future<void> _logSyncOperationInTransaction(
      Transaction txn, String tableName, String recordId, DatabaseOperation operation) async {
    final now = DateTime.now().millisecondsSinceEpoch;
