...
class SQLiteHelper {
  ...
  Future<void> performInitialDataMigration() async {
    try {
      ...
      if (adminUid.isEmpty) {
        final apiDAO = NodeApiDAO();
        final currentUser = await apiDAO.getCurrentUser();
        if (currentUser != null) {
      ...
      final isOnline = await apiDAO.isOnline();
      if (!isOnline) {
      ...
      // Migrate food items
      await _migrateFoodItems(apiDAO, db, adminUid);

      // Migrate departments
      await _migrateDepartments(apiDAO, db, adminUid);

      // Migrate recent bills (last 30 days)
      await _migrateRecentBills(apiDAO, db, adminUid);
