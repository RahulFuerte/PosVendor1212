  ...
  group('Final Performance Testing and Validation', () {
    ...
    group('Realistic Data Volume Performance Testing', () {
      test('should handle large realistic dataset efficiently', () async {
        ...
        expect(deptCreationTime, lessThan(2000), 
          reason: 'Department creation should complete within 2 seconds');
        expect(itemCreationTime, lessThan(60000), 
          reason: 'Food item creation should complete within 60 seconds');
        expect(billCreationTime, lessThan(20000), 
          reason: 'Bill creation should complete within 20 seconds');
      ...
      test('should maintain query performance with large datasets', () async {
        ...
        final queryTests = <String, Future<void> Function()>{
          'getAllItems': () async {
            ...
            stopwatch.stop();
            expect(items.length, greaterThanOrEqualTo(itemCount));
            expect(stopwatch.elapsedMilliseconds, lessThan(2000), 
              reason: 'Get all items should complete within 2 seconds');
          },
          
          'getDepartmentItems': () async {
            final stopwatch = Stopwatch()..start();
            final items = await sqliteDAO.getFoodItems(testAdminUid, department: 'Department 2');
            stopwatch.stop();
            expect(items.length, equals(100)); // itemCount / departmentCount
            expect(stopwatch.elapsedMilliseconds, lessThan(200), 
              reason: 'Department query should complete within 200ms');
          },
          ...
          'searchQuery': () async {
            ...
            expect(stopwatch.elapsedMilliseconds, lessThan(500), 
              reason: 'Search query should complete within 500ms');
          },
          
          'individualLookup': () async {
            final stopwatch = Stopwatch()..start();
            final item = await sqliteDAO.getFoodItem(testAdminUid, 'perf_item_250');
            stopwatch.stop();
            expect(item, isNotNull);
            expect(stopwatch.elapsedMilliseconds, lessThan(50), 
              reason: 'Individual lookup should complete within 50ms');
          },
        };
      ...
      test('should handle concurrent operations with realistic load', () async {
                ...
                try {
                  ...
                  switch (operationType) {
                    ...
                    case 2: // Update operation
                      await sqliteDAO.updateFoodItem(testAdminUid, 'concurrent_item_${op % 50}', {
                        'price': 20.0 + op,
                      });
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(60000), 
          reason: 'Concurrent operations should complete within 60 seconds');

        // Verify data integrity after concurrent operations
        final finalItems = await sqliteDAO.getFoodItems(testAdminUid);
        expect(finalItems.length, greaterThanOrEqualTo(50), 
    ...
    group('Complete Offline Functionality Validation', () {
      test('should handle complete offline workflow across all features', () async {
        ...
        for (int i = 0; i < 15; i++) {
          final foodItem = {
            ...
            'price': 8.0 + (i * 0.5),
            'department': 'Offline Department ${i % 3}',
        ...
        for (int i = 0; i < 5; i++) {
          final bill = {
            ...
            'items': '{"offline_food_${i % 15}": {"name": "Offline Food Item ${i % 15}", "price": ${8.0 + (i * 0.5)}}}',
        ...
        expect(offlineDepartments.length, equals(3));
        ...
        expect(offlineFoodItems.length, equals(12)); // 15 created - 3 deleted

        final allBills = await sqliteDAO.getBills(testAdminUid);
        final offlineBills = allBills.where((b) => 
          (b['id'] as String).startsWith('offline_bill_')).toList();
        expect(offlineBills.length, equals(5));
        ...
        expect(pendingDepartments.length, equals(3));
      ...
      test('should maintain offline performance under load', () async {
        ...
        expect(avgTime, lessThan(100), 
          reason: 'Average offline operation should be under 100ms');
        expect(maxTime, lessThan(500), 
          reason: 'Maximum offline operation should be under 500ms');
    ...
    group('Query Performance Improvements Validation', () {
      test('should demonstrate significant performance improvements with optimizations', () async {
        ...
        final indexedQueryTests = <String, Future<void> Function()>{
          'departmentQuery': () async {
            ...
            expect(items.length, equals(100)); // itemCount / 10 departments
            expect(stopwatch.elapsedMilliseconds, lessThan(100), 
              reason: 'Indexed department query should be under 100ms');
          },
          
          'primaryKeyLookup': () async {
            final stopwatch = Stopwatch()..start();
            final item = await sqliteDAO.getFoodItem(testAdminUid, 'opt_item_500');
            stopwatch.stop();
            
            expect(item, isNotNull);
            expect(stopwatch.elapsedMilliseconds, lessThan(20), 
              reason: 'Primary key lookup should be under 20ms');
          },
          ...
          'paginatedWithIndex': () async {
            ...
            expect(items.length, equals(50));
            expect(stopwatch.elapsedMilliseconds, lessThan(50), 
              reason: 'Paginated query with index should be under 50ms');
          },
          ...
          'fullTextSearch': () async {
            for (final searchTerm in searchTerms) {
              ...
              expect(stopwatch.elapsedMilliseconds, lessThan(300), 
                reason: 'FTS search for "$searchTerm" should be under 300ms');
            }
          },
        };
        ...
        batchStopwatch.stop();
        expect(batchStopwatch.elapsedMilliseconds, lessThan(5000), 
          reason: 'Batch updates should complete within 5 seconds');

        // Test query result caching effectiveness
        final cacheTestStopwatch = Stopwatch()..start();
        
        // First query (cache miss)
        await sqliteDAO.getFoodItems(testAdminUid, department: 'Department 3');
        ...
        expect(secondQueryTime, lessThanOrEqualTo(firstQueryTime + 20), 
      ...
      test('should validate prepared statement performance', () async {
        ...
        for (int i = 0; i < queryCount; i++) {
          ...
          await sqliteDAO.getFoodItem(testAdminUid, 'prep_item_${i % 25}');
        ...
        expect(avgTime, lessThan(30), 
          reason: 'Average prepared statement query should be under 30ms');
        
        // Performance should be consistent or improve over time with prepared statements
        expect(lastTenAvg, lessThanOrEqualTo(firstTenAvg * 2.0), 
          reason: 'Query performance should remain reasonably consistent with prepared statements');
      });
    });
      ...
      test('should validate complete system health under realistic conditions', () async {
        ...
        final healthChecks = <String, Future<void> Function()>{
          ...
          'queryPerformance': () async {
            ...
            await sqliteDAO.getFoodItems(testAdminUid, department: 'Health Department 1');
            await sqliteDAO.searchFoodItems(testAdminUid, 'Health');
            await sqliteDAO.getFoodItemsPaginated(testAdminUid, limit: 25);
            await sqliteDAO.getFoodItem(testAdminUid, 'health_item_150');
            
            stopwatch.stop();
            expect(stopwatch.elapsedMilliseconds, lessThan(1000), 
              reason: 'Mixed query workload should complete within 1 second');
          },
        ...
        // 4. Final system validation - verify all data is accessible
        final finalItemCount = await sqliteDAO.getFoodItemsCount(testAdminUid);
        expect(finalItemCount, greaterThanOrEqualTo(departments * itemsPerDept));
        
        final finalCacheStats = await imageCacheService.getCacheStatistics();
        expect(finalCacheStats['totalImages'], greaterThanOrEqualTo(images));
      });
    });
  });
}
